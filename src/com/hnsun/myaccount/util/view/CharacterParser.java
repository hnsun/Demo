...
public class CharacterParser {
	...
	public String convert(String str) { //单字解析
		String ret = null;
		UtilBoss.ConditionUtil.n(str);
		
		int ascii = getChsAscii(str);
		if(ascii > 0 && ascii < 160) ret = String.valueOf((char) ascii);
		else {
	...
	public String spellingStr(String chs) { //词组解析
		UtilBoss.ConditionUtil.n(chs);
		...
		for(int i = 0; i < chs.length(); i++) {
			...
			if(key.getBytes().length >= 2) {
				...
				if(UtilBoss.ObjUtil.isNull(value)) value = "unknown";
			} else value = key;
	...
	public String getSpelling() {
		return spellingStr(getResource());
	}

	private int getChsAscii(String chs) { //汉字转成ASCII码
		int ret = 0;
		try {
			byte[] bytes = chs.getBytes("gb2312");
			if(UtilBoss.ObjUtil.isNull(bytes) || bytes.length > 2 || bytes.length <= 0) throw new RuntimeException("illegal resource string");
			if(bytes.length == 1) ret = bytes[0];
			if(bytes.length == 2) {
				int hightByte = 256 + bytes[0];
				int lowByte = 256 + bytes[1];
				ret = (256 * hightByte + lowByte) - 256 * 256;
			}
		} catch(Exception e) {
			LogFactory.log().e(e, "ERROR:ChineseSpelling.class-getChsAscii(String chs)");
		}
		return ret;
	}
	...
	private static CharacterParser instance; //单例实例
	...
	static {
		CharacterParser.instance = new CharacterParser();
	}
}
